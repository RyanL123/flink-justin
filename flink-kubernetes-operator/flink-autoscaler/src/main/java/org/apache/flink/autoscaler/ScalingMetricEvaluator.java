...
public class ScalingMetricEvaluator {
    ...
    /**
     * Generate a memory-parallelism curve based on the current vertex metrics using a heuristic.
     *
     * <p>The heuristic assumes a constant scaling ratio of 0.7: when parallelism increases by 1,
     * memory need decreases by 30%. Conversely, when parallelism decreases by 1, memory need
     * increases by approximately 43% (1/0.7).
     *
     * <p>The curve is generated by:
     * <ol>
     *   <li>Using the current parallelism and managed memory as a known point on the curve</li>
     *   <li>Extrapolating to other parallelism levels using the 0.7 ratio</li>
     * </ol>
     *
     * @param currentParallelism the current parallelism of the vertex
     * @param currentMemoryMB the current managed memory used by the vertex in MB
     * @param targetThroughput the target throughput for this curve
     * @param conf the autoscaler configuration
     * @return a MemoryParallelismCurve based on the heuristic
     */
    private MemoryParallelismCurve generateMemoryParallelismCurveFromHeuristic(
            int currentParallelism,
            double currentMemoryMB,
            double targetThroughput,
            Configuration conf) {

        // Scaling ratio: memory at (p+1) = memory at (p) * SCALING_RATIO
        final double SCALING_RATIO = 0.7;
        ...
        for (int p = minParallelism; p <= maxParallelism; p++) {
            ...
            // Formula: memory(p) = currentMemory * SCALING_RATIO^(p - currentParallelism)
